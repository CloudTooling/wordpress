...
* Please ensure that the test suite passes **and** that code is lint free before submitting a PR by running:
  * Verify docker build working ```docker build .```
 * ``test/shell/.bin/run-tests.sh```
* If you've added new functionality, **please** include bats tests which validate its behaviour
* Make reference to possible [issues](https://github.com/CloudTooling/dev-buildboxissues) on PR comment
...
* All usage changes (e.g. docker entrypoint etc.) **must be documented**.


## <a name="commit"></a> Git Commit Guidelines

We're using [Conventional Commit Guidelines](https://www.conventionalcommits.org/en/v1.0.0/#summary)
